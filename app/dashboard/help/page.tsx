...
export default function HelpPage() {
  ...
  async function sendFeedback(e: React.FormEvent) {
    ...
    setSending(true); setFeedbackStatus('idle')
    try {
      const res = await fetch('/api/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: feedbackMsg, type: 'support' }),
      })
  ...
  return (
    <div style={{ maxWidth: '860px' }}>
      ...
      {/* Contact support */}
      <div style={{ marginTop: '24px', padding: '20px', background: '#0F1117', border: '1px solid #1E2028', borderRadius: '12px' }}>
        ...
        <form onSubmit={sendFeedback} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {feedbackStatus === 'error' && (
            <div style={{ fontSize: '13px', color: '#f87171' }}>Failed to send. Please try zaifee@cvidsproductions.net directly.</div>
          )}
          <button
            type="submit"
            disabled={sending || !feedbackMsg.trim()}
            style={{ alignSelf: 'flex-start', padding: '9px 22px', background: '#AAFF00', color: '#080A0E', border: 'none', borderRadius: '8px', fontWeight: 700, fontSize: '13px', cursor: 'pointer', opacity: sending || !feedbackMsg.trim() ? 0.6 : 1, fontFamily: 'Outfit, sans-serif' }}
          >
